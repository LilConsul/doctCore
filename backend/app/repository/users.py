...
class UsersRepository(BaseRepo):
    ...
    async def get_user_profile_by_email(email: str):
        sql = text(
            f"SELECT name, email, phone, role, sex FROM {UsersRepository.table_name} WHERE email = :email"
        )
    ...
    async def update_password(email: str, password: str):
        sql = text(
            f"UPDATE {UsersRepository.table_name} SET password = :password WHERE email = :email"
        )
    ...
    async def update_profile(email: str, **kwargs):
        ...
        sql = text(
            f"UPDATE {UsersRepository.table_name} SET {set_clause} WHERE email = :email"
        )
