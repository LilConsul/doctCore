...
async def get_all_medical_records(
    ...
    if user["role"] != "doctor":
        return ResponseSchema(
            detail="Only doctors can view medical records", result=None
        )
...
async def create_medical_record(
    ...
    _result = await MedicalRecordRepository.create_medical_record(
        medical_record, doctor_email=token["email"]
    )
