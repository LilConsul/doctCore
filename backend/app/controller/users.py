...
router = APIRouter(
    prefix="/users", tags=["User"], dependencies=[Depends((JWTBearer()))]
)
...
async def get_user_profile(
    ...
    return ResponseSchema(
        detail="User profile retrieved successfully", result=result_dict
    )
...
async def get_patient_profile(
    ...
    result_dict = dict(_result) if not isinstance(_result, dict) else _result
    return ResponseSchema(
        detail="Patient profile retrieved successfully", result=result_dict
    )
...
async def get_doctor_profile(
    ...
    result_dict = dict(_result) if not isinstance(_result, dict) else _result
    return ResponseSchema(
        detail="Doctor profile retrieved successfully", result=result_dict
    )


@router.put(
    "/update-profile", response_model=ResponseSchema, response_model_exclude_none=True
)
async def update_user_profile(
    user: EmptyUser, credentials: HTTPAuthorizationCredentials = Security(JWTBearer())
) -> ResponseSchema:
    token = JWTRepo.extract_token(credentials)
    _result = await UsersRepository.update_profile(
        token["email"], **user.dict(exclude_none=True)
    )
    if not _result:
        return ResponseSchema(status_code=404, detail="User not found")
    return ResponseSchema(
        detail="User profile updated successfully", result=dict(_result)
    )


@router.get(
    "/list-patients", response_model=ResponseSchema, response_model_exclude_none=True
)
