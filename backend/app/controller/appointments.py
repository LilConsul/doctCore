...
async def create_appointment(appointment: AppointmentSchema, credentials: HTTPAuthorizationCredentials = Security(JWTBearer())) -> ResponseSchema:
    token = JWTRepo.extract_token(credentials)
    _result = None
    _appointment = dict(appointment)
    if token['role'] == 'patient':
        _result = await AppointmentsRepository.create_appointment(_appointment, patient_email=token['email'])
    if token['role'] == 'doctor':
        _result = await AppointmentsRepository.create_appointment(_appointment, doctor_email=token['email'])
...
async def approve_appointment(appointment_id: int, status: AppointmentStatus, credentials: HTTPAuthorizationCredentials = Security(JWTBearer())) -> ResponseSchema:
    token = JWTRepo.extract_token(credentials)
    user = await UsersRepository.get_user_profile_by_email(token['email'])
    if user['role'] != 'doctor':
        return ResponseSchema(detail="Only doctors can change appointments", result=None)
    ...
    result = await AppointmentsRepository.update_appointment_status(appointment_id, status.value)
...
async def get_appointment_history(credentials: HTTPAuthorizationCredentials = Security(JWTBearer())) -> ResponseSchema:
    token = JWTRepo.extract_token(credentials)
    user = await UsersRepository.get_user_profile_by_email(token['email'])
    if user['role'] != 'doctor':
        return ResponseSchema(detail="Only doctors can view appointment history", result=None)

    appointments = await AppointmentsRepository.get_history_by_doctor_email(token['email'])
    _appointments = [dict(appointment) for appointment in appointments] if appointments else []
    return ResponseSchema(detail="Appointments found", result=_appointments)